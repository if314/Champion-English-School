...
def schedule_main(lang):
    ...
    summary = f"""<div class="card" style="padding:26px 28px">
      <div class="contact-list">
        ...
        <div class="contact-row"><span class="ic" aria-hidden="true">💳</span><div><div class="lbl">{t("Цена", "Price", lang)}</div><span class="val"><s style="color:var(--muted);font-weight:600">{t("399 €", "€399", lang)}</s> {t("329 € за учебен срок", "€329 per school term", lang)}</span><div style="font-size:13px;color:var(--muted);font-weight:500;margin-top:4px">{t(f"≈ {329/60:.2f}".replace(".", ",") + " € на учебен час (60 уч. часа на срок)", f"≈ €{329/60:.2f} per teaching hour (60 teaching hours per term)", lang)}</div><div style="font-size:13px;color:var(--muted);font-weight:500;margin-top:4px">{t("Учебниците не са включени в цената на курса и се заплащат при записване.", "Textbooks are not included in the course price and are paid for separately at enrollment.", lang)}</div></div></div>
